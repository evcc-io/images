...
RPI_IMAGE_ZIP=$(echo "$RELEASE_INFO" | jq -r '.assets[] | select(.name | contains("raspberry-pi") and contains(".img.zip")) | .name')
RPI_IMAGE_SHA=$(echo "$RELEASE_INFO" | jq -r '.assets[] | select(.name | contains("raspberry-pi") and contains(".img.sha")) | .name')
